...
const { REDIS_HOST, REDIS_PORT = 6379, REDIS_PASSWORD, BRIGHTDATA_TOKEN, BD_DS_GOOGLE } = process.env;
...
async function trigger(prompt, locale){
  ...
  const params=new URLSearchParams({ dataset_id: BD_DS_GOOGLE, include_errors:'true' });
  const body=[ { url:'https://google.com/aimode', prompt, country: locale||'' } ];
...
async function runJob({prompt, locale='US'}){ if(!BRIGHTDATA_TOKEN) throw new Error('Missing BRIGHTDATA_TOKEN'); if(!BD_DS_GOOGLE) throw new Error('Missing BD_DS_GOOGLE'); const snap=await trigger(prompt, locale); const raw=await waitSnap(snap); const answer=pickAnswer(raw); return { engine:'google', provider:'brightdata', provider_snapshot_id:snap, answer, raw }; }
...
console.log('worker.google started');
