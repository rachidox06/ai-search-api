...
const { REDIS_HOST, REDIS_PORT = 6379, REDIS_PASSWORD, BRIGHTDATA_TOKEN, BD_DS_PERPLEXITY } = process.env;
...
async function trigger(prompt){
  ...
  const params=new URLSearchParams({ dataset_id: BD_DS_PERPLEXITY, include_errors:'true' });
  const body=[ { url:'https://www.perplexity.ai', prompt, index:1 } ]; // per your example
...
async function runJob({prompt, locale='US'}){ if(!BRIGHTDATA_TOKEN) throw new Error('Missing BRIGHTDATA_TOKEN'); if(!BD_DS_PERPLEXITY) throw new Error('Missing BD_DS_PERPLEXITY'); const snap=await trigger(prompt); const raw=await waitSnap(snap); const answer=pickAnswer(raw); return { engine:'perplexity', provider:'brightdata', provider_snapshot_id:snap, answer, raw }; }
...
console.log('worker.perplexity started');
